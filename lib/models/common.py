    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                #print(y[..., 2:4].shape)
                #print(torch.pow(torch.mul(y[..., 2:4], 2),2 ).shape )
                #anchors_broadcast = torch.broadcast_to(self.anchor_grid[i], y[..., 2:4].shape)
                #res2 = torch.concat([self.anchor_grid[i] for _ in range(y[..., 2:4].shape[3] ) ], (3) )#.shape
                #anchors_broadcast = torch.concat([res2 for _ in range(y[..., 2:4].shape[2]) ], (2) )

                #out_shape = y[..., 2:4].shape
                #anchors_broadcast = self.anchor_grid[i].repeat(1,1, out_shape[2] ,out_shape[3], 1)

                #anchors_broadcast = torch.broadcast_to(self.anchor_grid[i], y[..., 2:4].shape)
                #y[..., 2:4] = torch.mul(torch.pow(torch.mul(y[..., 2:4], 2), 2) , anchors_broadcast )
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh                

                #y[..., 2:4] = torch.mul(torch.pow(torch.mul(y[..., 2:4], 2), 2) , self.anchor_grid[i])  # wh
                """print("**")
                print(y.shape)  #[1, 3, w, h, 85]
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=13, anchors=(), ch=(), inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                #print('AAAAAAAA:', y.shape)
                #print( self.stride[i].shape)
                #print('bbbbbbbb:', (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device) ) * self.stride[i] )
                #print(  self.anchor_grid[i].shape)
                #print('cccccccc:', (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i])
                #y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
                #y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh

                y[..., 0:2] = torch.mul(torch.add(torch.sub(torch.mul(y[..., 0:2], 2.), 0.5),  self.grid[i].to(x[i].device)),  self.stride[i])  # xy
                y[..., 2:4] = torch.mul(torch.pow(torch.mul(y[..., 2:4],  2), 2),  self.anchor_grid[i])  # wh
                """print("**")
                print(y.shape)  #[1, 3, w, h, 85]
                print(y.view(bs, -1, self.no).shape) #[1, 3*w*h, 85]"""
