...
class MultiHeadLoss(nn.Module):
    ...
    def forward(self, head_fields, head_targets, shapes, model, bdd=True):
        """
        Inputs:
        ...
        # print(model.nc)
        total_loss, head_losses = self._forward_impl(head_fields, head_targets, shapes, model, bdd=bdd)

        return total_loss, head_losses

    def _forward_impl(self, predictions, targets, shapes, model, bdd=True):
        """
        ...
        if not bdd:
            ...
            tcls, tbox, indices, anchors = build_targets(cfg, predictions[0], targets[0], model, bdd)  # targets
            ...
            for i, pi in enumerate(predictions[0]):  # layer index, layer predictions
                ...
                if n:
                    ...
                    # Classification
                    # print(model.nc)
                    n_classes = ( model.module.num_classes_bdd if is_parallel(model) else model.num_classes_bdd ) if bdd \
                                    else  (model.module.num_classes_bosch if is_parallel(model) else  model.num_classes_bosch ) 
        ...
        if bdd:
            lcls = torch.tensor(0.).to(lseg_da.get_device())
            lobj = torch.tensor(0.).to(lseg_da.get_device())
            lbox = torch.tensor(0.).to(lseg_da.get_device())
            ...
            lseg_da = torch.tensor(0.).to(lbox.get_device())
            lseg_ll = torch.tensor(0.).to(lbox.get_device())
            liou_ll = torch.tensor(0.).to(lbox.get_device())
